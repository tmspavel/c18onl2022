...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User findByEmailAndPassword(String email, String password) {
        User user = null;
        try {
            user = jdbc.queryForObject(GET_USER_BY_EMAIL_AND_PASS, (RowMapper<User>) (rs, rowNum) -> User.builder()
                    .id(rs.getInt("id"))
                    .name(rs.getString("name"))
                    .surname(rs.getString("surname"))
                    .email(rs.getString("email"))
            ...
            log.error("Problem in findByEmailAndPassword method: {}", e.getMessage());
        }
        return user;
    }
}
